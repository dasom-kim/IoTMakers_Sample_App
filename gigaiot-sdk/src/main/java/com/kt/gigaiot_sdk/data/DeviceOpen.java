package com.kt.gigaiot_sdk.data;

import com.google.gson.annotations.SerializedName;

/**
 * Created by ceoko on 15. 3. 24..
 */
public class DeviceOpen {

    @SerializedName("oprtSttusCd")
    private String oprtSttusCd;

    @SerializedName("svcContId")
    private String svcContId;

    @SerializedName("svcTgtSeq")
    private String svcTgtSeq;

    @SerializedName("dstrCd")
    private String dstrCd;

    @SerializedName("svcThemeCd")
    private String svcThemeCd;

    @SerializedName("unitSvcCd")
    private String unitSvcCd;

    @SerializedName("spotDevSeq")
    private String spotDevSeq;

    @SerializedName("spotDevId")
    private String spotDevId;

    @SerializedName("devUUID")
    private String devUUID;

    @SerializedName("devNm")
    private String devNm;

    @SerializedName("devPwd")
    private String devPwd;

    @SerializedName("gwCnctId")
    private String gwCnctId;

    @SerializedName("gwCnctDivCd")
    private String gwCnctDivCd;

    @SerializedName("upSpotDevId")
    private String upSpotDevId;

    @SerializedName("upSpotDevSeq")
    private String upSpotDevSeq;

    @SerializedName("latitVal")
    private String latitVal;

    @SerializedName("latitDivCd")
    private String latitDivCd;

    @SerializedName("lngitVal")
    private String lngitVal;

    @SerializedName("lngitDivCd")
    private String lngitDivCd;

    @SerializedName("altt")
    private String altt;

    @SerializedName("colecCyclTime")
    private String colecCyclTime;

    @SerializedName("stdProtTypeYn")
    private String stdProtTypeYn;

    @SerializedName("stdProtTypeNm")
    private String stdProtTypeNm;

    @SerializedName("ottpYn")
    private String ottpYn;

    @SerializedName("useYn")
    private String useYn;

    @SerializedName("delYn")
    private String delYn;

    @SerializedName("physDevYn")
    private String physDevYn;

    @SerializedName("tmpDevYn")
    private String tmpDevYn;

    @SerializedName("frmwrVerNo")
    private String frmwrVerNo;

    @SerializedName("athnNo")
    private String athnNo;

    @SerializedName("athnFormlCd")
    private String athnFormlCd;

    @SerializedName("devSttusCd")
    private String devSttusCd;

    @SerializedName("subsSttusCd")
    private String subsSttusCd;

    @SerializedName("devConnTypeCd")
    private String devConnTypeCd;

    @SerializedName("troblAdmrId")
    private String troblAdmrId;

    @SerializedName("eqpLoSbst")
    private String eqpLoSbst;

    @SerializedName("rmark")
    private String rmark;

    @SerializedName("cretrId")
    private String cretrId;

    @SerializedName("cretDt")
    private String cretDt;

    @SerializedName("amdrId")
    private String amdrId;

    @SerializedName("amdDt")
    private String amdDt;

    @SerializedName("devModelSeq")
    private String devModelSeq;

    @SerializedName("devModelCd")
    private String devModelCd;

    @SerializedName("devModelNm")
    private String devModelNm;

    @SerializedName("termlMakrNm")
    private String termlMakrNm;

    @SerializedName("protId")
    private String protId;

    @SerializedName("bindTypeCd")
    private String bindTypeCd;

    @SerializedName("protNm")
    private String protNm;

    @SerializedName("bindTypeNm")
    private String bindTypeNm;

    @SerializedName("atcFileSeq")
    private String atcFileSeq;

    @SerializedName("atcFileNm")
    private String atcFileNm;

    @SerializedName("atcFileId")
    private String atcFileId;

    @SerializedName("smltSeq")
    private String smltSeq;

    @SerializedName("sttus")
    private String sttus;

    public String getOprtSttusCd() {
        return oprtSttusCd;
    }

    public void setOprtSttusCd(String oprtSttusCd) {
        this.oprtSttusCd = oprtSttusCd;
    }

    public String getSvcContId() {
        return svcContId;
    }

    public void setSvcContId(String svcContId) {
        this.svcContId = svcContId;
    }

    public String getSvcTgtSeq() {
        return svcTgtSeq;
    }

    public void setSvcTgtSeq(String svcTgtSeq) {
        this.svcTgtSeq = svcTgtSeq;
    }

    public String getDstrCd() {
        return dstrCd;
    }

    public void setDstrCd(String dstrCd) {
        this.dstrCd = dstrCd;
    }

    public String getSvcThemeCd() {
        return svcThemeCd;
    }

    public void setSvcThemeCd(String svcThemeCd) {
        this.svcThemeCd = svcThemeCd;
    }

    public String getUnitSvcCd() {
        return unitSvcCd;
    }

    public void setUnitSvcCd(String unitSvcCd) {
        this.unitSvcCd = unitSvcCd;
    }

    public String getSpotDevSeq() {
        return spotDevSeq;
    }

    public void setSpotDevSeq(String spotDevSeq) {
        this.spotDevSeq = spotDevSeq;
    }

    public String getSpotDevId() {
        return spotDevId;
    }

    public void setSpotDevId(String spotDevId) {
        this.spotDevId = spotDevId;
    }

    public String getDevUUID() {
        return devUUID;
    }

    public void setDevUUID(String devUUID) {
        this.devUUID = devUUID;
    }

    public String getDevNm() {
        return devNm;
    }

    public void setDevNm(String devNm) {
        this.devNm = devNm;
    }

    public String getDevPwd() {
        return devPwd;
    }

    public void setDevPwd(String devPwd) {
        this.devPwd = devPwd;
    }

    public String getGwCnctId() {
        return gwCnctId;
    }

    public void setGwCnctId(String gwCnctId) {
        this.gwCnctId = gwCnctId;
    }

    public String getGwCnctDivCd() {
        return gwCnctDivCd;
    }

    public void setGwCnctDivCd(String gwCnctDivCd) {
        this.gwCnctDivCd = gwCnctDivCd;
    }

    public String getUpSpotDevId() {
        return upSpotDevId;
    }

    public void setUpSpotDevId(String upSpotDevId) {
        this.upSpotDevId = upSpotDevId;
    }

    public String getUpSpotDevSeq() {
        return upSpotDevSeq;
    }

    public void setUpSpotDevSeq(String upSpotDevSeq) {
        this.upSpotDevSeq = upSpotDevSeq;
    }

    public String getLatitVal() {
        return latitVal;
    }

    public void setLatitVal(String latitVal) {
        this.latitVal = latitVal;
    }

    public String getLatitDivCd() {
        return latitDivCd;
    }

    public void setLatitDivCd(String latitDivCd) {
        this.latitDivCd = latitDivCd;
    }

    public String getLngitVal() {
        return lngitVal;
    }

    public void setLngitVal(String lngitVal) {
        this.lngitVal = lngitVal;
    }

    public String getLngitDivCd() {
        return lngitDivCd;
    }

    public void setLngitDivCd(String lngitDivCd) {
        this.lngitDivCd = lngitDivCd;
    }

    public String getAltt() {
        return altt;
    }

    public void setAltt(String altt) {
        this.altt = altt;
    }

    public String getColecCyclTime() {
        return colecCyclTime;
    }

    public void setColecCyclTime(String colecCyclTime) {
        this.colecCyclTime = colecCyclTime;
    }

    public String getStdProtTypeYn() {
        return stdProtTypeYn;
    }

    public void setStdProtTypeYn(String stdProtTypeYn) {
        this.stdProtTypeYn = stdProtTypeYn;
    }

    public String getStdProtTypeNm() {
        return stdProtTypeNm;
    }

    public void setStdProtTypeNm(String stdProtTypeNm) {
        this.stdProtTypeNm = stdProtTypeNm;
    }

    public String getOttpYn() {
        return ottpYn;
    }

    public void setOttpYn(String ottpYn) {
        this.ottpYn = ottpYn;
    }

    public String getUseYn() {
        return useYn;
    }

    public void setUseYn(String useYn) {
        this.useYn = useYn;
    }

    public String getDelYn() {
        return delYn;
    }

    public void setDelYn(String delYn) {
        this.delYn = delYn;
    }

    public String getPhysDevYn() {
        return physDevYn;
    }

    public void setPhysDevYn(String physDevYn) {
        this.physDevYn = physDevYn;
    }

    public String getTmpDevYn() {
        return tmpDevYn;
    }

    public void setTmpDevYn(String tmpDevYn) {
        this.tmpDevYn = tmpDevYn;
    }

    public String getFrmwrVerNo() {
        return frmwrVerNo;
    }

    public void setFrmwrVerNo(String frmwrVerNo) {
        this.frmwrVerNo = frmwrVerNo;
    }

    public String getAthnNo() {
        return athnNo;
    }

    public void setAthnNo(String athnNo) {
        this.athnNo = athnNo;
    }

    public String getAthnFormlCd() {
        return athnFormlCd;
    }

    public void setAthnFormlCd(String athnFormlCd) {
        this.athnFormlCd = athnFormlCd;
    }

    public String getDevSttusCd() {
        return devSttusCd;
    }

    public void setDevSttusCd(String devSttusCd) {
        this.devSttusCd = devSttusCd;
    }

    public String getSubsSttusCd() {
        return subsSttusCd;
    }

    public void setSubsSttusCd(String subsSttusCd) {
        this.subsSttusCd = subsSttusCd;
    }

    public String getDevConnTypeCd() {
        return devConnTypeCd;
    }

    public void setDevConnTypeCd(String devConnTypeCd) {
        this.devConnTypeCd = devConnTypeCd;
    }

    public String getTroblAdmrId() {
        return troblAdmrId;
    }

    public void setTroblAdmrId(String troblAdmrId) {
        this.troblAdmrId = troblAdmrId;
    }

    public String getEqpLoSbst() {
        return eqpLoSbst;
    }

    public void setEqpLoSbst(String eqpLoSbst) {
        this.eqpLoSbst = eqpLoSbst;
    }

    public String getRmark() {
        return rmark;
    }

    public void setRmark(String rmark) {
        this.rmark = rmark;
    }

    public String getCretrId() {
        return cretrId;
    }

    public void setCretrId(String cretrId) {
        this.cretrId = cretrId;
    }

    public String getCretDt() {
        return cretDt;
    }

    public void setCretDt(String cretDt) {
        this.cretDt = cretDt;
    }

    public String getAmdrId() {
        return amdrId;
    }

    public void setAmdrId(String amdrId) {
        this.amdrId = amdrId;
    }

    public String getAmdDt() {
        return amdDt;
    }

    public void setAmdDt(String amdDt) {
        this.amdDt = amdDt;
    }

    public String getDevModelSeq() {
        return devModelSeq;
    }

    public void setDevModelSeq(String devModelSeq) {
        this.devModelSeq = devModelSeq;
    }

    public String getDevModelCd() {
        return devModelCd;
    }

    public void setDevModelCd(String devModelCd) {
        this.devModelCd = devModelCd;
    }

    public String getDevModelNm() {
        return devModelNm;
    }

    public void setDevModelNm(String devModelNm) {
        this.devModelNm = devModelNm;
    }

    public String getTermlMakrNm() {
        return termlMakrNm;
    }

    public void setTermlMakrNm(String termlMakrNm) {
        this.termlMakrNm = termlMakrNm;
    }

    public String getProtId() {
        return protId;
    }

    public void setProtId(String protId) {
        this.protId = protId;
    }

    public String getBindTypeCd() {
        return bindTypeCd;
    }

    public void setBindTypeCd(String bindTypeCd) {
        this.bindTypeCd = bindTypeCd;
    }

    public String getProtNm() {
        return protNm;
    }

    public void setProtNm(String protNm) {
        this.protNm = protNm;
    }

    public String getBindTypeNm() {
        return bindTypeNm;
    }

    public void setBindTypeNm(String bindTypeNm) {
        this.bindTypeNm = bindTypeNm;
    }

    public String getAtcFileSeq() {
        return atcFileSeq;
    }

    public void setAtcFileSeq(String atcFileSeq) {
        this.atcFileSeq = atcFileSeq;
    }

    public String getAtcFileNm() {
        return atcFileNm;
    }

    public void setAtcFileNm(String atcFileNm) {
        this.atcFileNm = atcFileNm;
    }

    public String getAtcFileId() {
        return atcFileId;
    }

    public void setAtcFileId(String atcFileId) {
        this.atcFileId = atcFileId;
    }

    public String getSmltSeq() {
        return smltSeq;
    }

    public void setSmltSeq(String smltSeq) {
        this.smltSeq = smltSeq;
    }

    public String getSttus() {
        return sttus;
    }

    public void setSttus(String sttus) {
        this.sttus = sttus;
    }

    @Override
    public String toString() {
        return "DeviceOpen{" +
                "oprtSttusCd='" + oprtSttusCd + '\'' +
                "svcContId='" + svcContId + '\'' +
                "svcTgtSeq='" + svcTgtSeq + '\'' +
                "dstrCd='" + dstrCd + '\'' +
                "svcThemeCd='" + svcThemeCd + '\'' +
                "unitSvcCd='" + unitSvcCd + '\'' +
                "spotDevSeq='" + spotDevSeq + '\'' +
                "spotDevId='" + spotDevId + '\'' +
                "devUUID='" + devUUID + '\'' +
                "devNm='" + devNm + '\'' +
                "devPwd='" + devPwd + '\'' +
                "gwCnctId='" + gwCnctId + '\'' +
                "gwCnctDivCd='" + gwCnctDivCd + '\'' +
                "upSpotDevId='" + upSpotDevId + '\'' +
                "upSpotDevSeq='" + upSpotDevSeq + '\'' +
                "latitVal='" + latitVal + '\'' +
                "latitDivCd='" + latitDivCd + '\'' +
                "lngitVal='" + lngitVal + '\'' +
                "lngitDivCd='" + lngitDivCd + '\'' +
                "altt='" + altt + '\'' +
                "colecCyclTime='" + colecCyclTime + '\'' +
                "stdProtTypeYn='" + stdProtTypeYn + '\'' +
                "stdProtTypeNm='" + stdProtTypeNm + '\'' +
                "ottpYn='" + ottpYn + '\'' +
                "useYn='" + useYn + '\'' +
                "delYn='" + delYn + '\'' +
                "physDevYn='" + physDevYn + '\'' +
                "tmpDevYn='" + tmpDevYn + '\'' +
                "frmwrVerNo='" + frmwrVerNo + '\'' +
                "athnNo='" + athnNo + '\'' +
                "athnFormlCd='" + athnFormlCd + '\'' +
                "devSttusCd='" + devSttusCd + '\'' +
                "subsSttusCd='" + subsSttusCd + '\'' +
                "devConnTypeCd='" + devConnTypeCd + '\'' +
                "troblAdmrId='" + troblAdmrId + '\'' +
                "eqpLoSbst='" + eqpLoSbst + '\'' +
                "rmark='" + rmark + '\'' +
                "cretrId='" + cretrId + '\'' +
                "cretDt='" + cretDt + '\'' +
                "amdrId='" + amdrId + '\'' +
                "amdDt='" + amdDt + '\'' +
                "devModelSeq='" + devModelSeq + '\'' +
                "devModelCd='" + devModelCd + '\'' +
                "devModelNm='" + devModelNm + '\'' +
                "termlMakrNm='" + termlMakrNm + '\'' +
                "protId='" + protId + '\'' +
                "bindTypeCd='" + bindTypeCd + '\'' +
                "protNm='" + protNm + '\'' +
                "bindTypeNm='" + bindTypeNm + '\'' +
                "atcFileSeq='" + atcFileSeq + '\'' +
                "atcFileNm='" + atcFileNm + '\'' +
                "atcFileId='" + atcFileId + '\'' +
                "smltSeq='" + smltSeq + '\'' +
                "sttus='" + sttus + "'" +
                '}';
    }
}
