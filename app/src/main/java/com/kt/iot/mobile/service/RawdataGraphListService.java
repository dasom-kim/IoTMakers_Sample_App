...
public class RawdataGraphListService {
    ...
    public interface UpdateListener {
        void onUpdate(List<LogStream> list);
    }
    ...
    public void startPolling(DeviceNew device) {
    ...
    class LogPollingThread extends Thread {
        ...
        LogPollingThread(DeviceNew device) {
            this.device = device;
        }
        ...
        public void run() {
            if (device != null ) {
                if (device.getTagStrmList() != null && device.getTagStrmList().size() > 0) {
                    while(isRunning) {
                        try {
                            final List<LogStream> logStreams = new ArrayList<LogStream>();
                            ...
                            tagStrmApi.getTagStrmLog(device.getId(), "100", "5");
                            Thread.sleep(10000);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
